...
public class AssertionMethods extends SelectElementByType implements BaseTest {
    ...
    /**
     * Method to verify if the element has focus
     *
     * @param accessType : String : Locator type (id, name, class, xpath, css)
     * @param accessName : String : Select element from dropdown by text or value
     * @param testCase   : Boolean : test case [true or false]
     * @throws TestCaseFailed
     */
    public void checkElementHasFocus(String accessType, String accessName, boolean testCase) throws TestCaseFailed {
        element = getWait().until(ExpectedConditions.presenceOfElementLocated(getelementbytype(accessType, accessName)));
        if (testCase) {
            if (!element.equals(getDriver().switchTo().activeElement()))
                throw new TestCaseFailed("Element does not have focus");
        } else {
            if (element.equals(getDriver().switchTo().activeElement()))
                throw new TestCaseFailed("Element has focus");
        }

    }

    public void checkCount(int number, String type, String accessName) throws TestCaseFailed {
        try {
            getWait().until(ExpectedConditions.numberOfElementsToBe(getelementbytype(type, accessName), number));
        } catch (TimeoutException e) {
            // Do nothing. Check unconditionally found elements.
        } finally {
            final int foundNum = getDriver().findElements(getelementbytype(type, accessName)).stream().filter(e -> e.isDisplayed()).collect(Collectors.toList()).size();
            if (foundNum != number)
